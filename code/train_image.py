...
def train(config):
    ...
    for i in tqdm.trange(config["num_iterations"]):
        ...
        optimizer_D_t.step()
    now = datetime.datetime.now()
    d = str(str(now.month)+'-'+str(now.day)+' '+str(now.hour)+':'+str(now.minute)+":"+str(now.second)).replace(':','-')
    torch.save(best_model, osp.join(config["output_path"],
                                    "{}_to_{}_best_model_acc-{}_{}.pth.tar".format(args.source, args.target,
                                                                            best_acc,d)))
    return best_acc
...
if __name__ == "__main__":
    ...
    parser.add_argument('--snapshot_interval', type=int, default=50, help="interval of two continuous output model")
    ...
    config["out_file"] = open(osp.join(config["output_path"], "log_{}_to_{}_{}.txt".format(args.source, args.target, str(datetime.datetime.utcnow()).replace(':','-'))),"w")
    if not osp.exists(config["output_path"]):
    ...
    config["data"] = {"source": {"list_path": args.s_dset_path, "batch_size": 9}, \
                      "target": {"list_path": args.t_dset_path, "batch_size": 9}, \
                      "test": {"list_path": args.t_dset_path, "batch_size": 1}}
